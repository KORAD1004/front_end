export const drawMap = (area, loading, setMap) => {
  if (!window.kakao || !window.kakao.maps || area.length === 0 || loading)
    return;
  ...
  const options = {
    center: new kakao.maps.LatLng(35.736, 127.8829),
    level: 13, // 지도의 확대 레벨
  ...
  kakao.maps.event.addListener(newMap, "dragend", () => {
    ...
    const newLat = Math.min(
      Math.max(currentCenter.getLat(), bounds.getSouthWest().getLat()),
      ...
      area.geometry.coordinates.forEach((polygon) => {
        ...
        kakao.maps.event.addListener(polygonObj, "click", function () {
          if (
            ...
            selectedPolygonRef.current.polygon !== polygonObj
          ) {
...
export const manufactureRadiation = (radiation, data, setArea) => {
  ...
  // JSON 데이터로 area 설정
  if (radiation && radiation.length > 0) {
    // 여기서 조건을 추가
  ...
  if (map) {
    ...
    if (map.getLevel() > 11 && isClick === false) {
      ...
      polygonObjects.current.forEach((polygon) => {
        if (gyeongjuPolygon.current !== polygon)
          polygon.setOptions({ fillColor: "#004c80" });
      });

      gyeongjuPolygon.current.setOptions({
        strokeColor: "#FF7676",
        strokeWeight: 5,
      });

      const gyeongjuData = area.find(
        (item) => item.properties.KOR_NM === "경주시",
      );
      if (gyeongjuData) {
        ...
        customOverlayRef.current.setPosition(
          new window.kakao.maps.LatLng(35.8388735, 129.196647),
        );
        customOverlayRef.current.setMap(map);
      }
    } else if (map.getLevel() < 13 && isClick === true) {
      map.setLevel(13);
      map.setCenter(new window.kakao.maps.LatLng(35.736, 127.8829));
      polygonObjects.current.forEach((polygonObj) => {
        const initialPolygon = initialPolygons.current.find(
          (item) => item.polygon === polygonObj,
        );
        if (initialPolygon) {
